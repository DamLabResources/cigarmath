...
def test_trim_alignment():
    ...
    # Test both ends
    new_start, new_cigars = trim_alignment(
        ref_start, cigartups, left=2, right=2)
    assert new_start == 12
...
def test_trim_alignment_with_clipping():
    ...
    new_start, new_cigars = trim_alignment(
        ref_start, cigartups, left=2, right=2, add_clipping='soft')
    assert new_start == 12
    assert tuple(new_cigars) == ((BAM_CSOFT_CLIP, 2),
                                 (0, 3), (2, 1), (0, 1), (BAM_CSOFT_CLIP, 2))

    # Test adding hard clipping
    new_start, new_cigars = trim_alignment(
        ref_start, cigartups, left=2, right=2, add_clipping='hard')
    assert new_start == 12
    assert tuple(new_cigars) == ((BAM_CHARD_CLIP, 2),
                                 (0, 3), (2, 1), (0, 1), (BAM_CHARD_CLIP, 2))
...
def test_combine_multiple_alignments():
    ...
    new_start, new_cigars = combine_multiple_alignments(
        overlapping, allowed_overlap=10)
